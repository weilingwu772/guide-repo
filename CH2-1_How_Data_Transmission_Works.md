...
| **MQTT**<br>*(Message Queuing Telemetry Transport)* | `Network Communication`<br>`Cloud & System Integration` | 發布/訂閱 <br>（Publish / Subscribe） | 輕量級訊息傳輸協定，無需裝置間直接連線，具有封包小、頻寬需求低等特性 | • 機器人與雲端平台資料交換<br>• IoT 感測器資料蒐集 |
...
```mermaid
graph TD
    subgraph Top["上層：整合上雲"]
        Cloud["雲端與系統整合<br>Cloud & System Integration"]
    end

    subgraph Mid["中間：傳輸基礎"]
        Net["網路傳輸<br>Network Communication"]
    end

    subgraph Bottom["底層：平行介接"]
        Vision["視覺與影像裝置<br>Vision & Imaging Devices"]
        Pos["定位裝置<br>Positioning Devices"]
        Industrial["控制器或工業設備<br>Controller & Industrial Devices"]
        Auto["車用系統<br>Automotive Systems"]
    end

    Net -->|提供 IP 網路傳輸| Cloud
    Net -->|提供影音串流傳輸| Vision
    Net -->|提供工業乙太網通訊| Industrial
    Net -->|提供車載乙太網通訊| Auto

    Pos -->|提供定位資訊| Industrial
    Pos -->|提供定位與導航資訊| Auto
    Industrial <-->|控制命令與設備狀態| Auto

    Vision -->|影音串流與影像管理| Cloud
    Industrial -->|設備狀態、監測與控制| Cloud
    Auto -->|車隊狀態與偵測資訊| Cloud
```

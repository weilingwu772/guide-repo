...
而目前硬體介面可依介接層級和使用方式不同，概分為板級介面與系統級介面兩種。前者是指系統單晶片（SoC）、載板（Carrier Board）與周邊模組間的短距離連接，如 MIPI、UART、I²C 與 SPI，通常存在於邊緣運算平台內部，大多無對外實體接頭，僅部分會透過擴充接頭對外提供；後者則是邊緣運算平台與外部裝置間的連接，如 GMSL、Ethernet、CAN、RS-485 與 USB，通常具有對外實體接頭，以因應系統整合需求。故以下依此分類順序，整理機器人開發上常見的硬體介面項目：

...
* **REP-120 Coordinate Frames for Humanoid Robots**（ [文件連結](https://www.ros.org/reps/rep-0120.html)）
    ...
    REP-120 延伸 REP-105 的座標框架概念，再針對**人型機器人（Humanoid Robot）**定義常用座標系的文件，除了建議人型機器人一樣採用 `base_link` 作為主要的機器人本體座標系，也進一步規範本體各部位重要座標框架的命名與意義，例如軀幹、手臂末端與足部等，以利後續依照機器人的實際機構建立各部位的 TF 關係，主要建議的座標系命名方式包括：
    ...
    值得一提的是，REP-120 定義了 `base_footprint` 座標系，用於表示機器人在地面上的位置，概念上可將其理解為機器人本體在地面上的「投影」，並使相對於 `odom` 的 `Roll` 與 `Pitch` 維持為 0，`Yaw` 則與 `base_link` 一致；如此一來，即使機器人在行走時因地面顛波產生上下起伏或身體傾斜，仍可透過 `base_footprint` 提供較穩定的 2D 平面位置與朝向，使導航相關演算法能專注於機器人在地面上的移動，而不必直接處理本體的高度與傾斜變化，同時，此概念在實務上也常被應用在輪型 AMR 的系統設計上。

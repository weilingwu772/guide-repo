...
延伸來看，這三大零組件群是彼此分工、相互協作的關係，以此構成一個完整的閉環系統：首選透過感知類零組件蒐集環境資訊，接著由運算類零組件進行即時決策，最後藉由執行類零組件完成指令動作，並同時回饋數據形成閉迴路，持續進行感知（P）、運算（C）和執行（E）的循環與調整。
...
| 模組類別 | 零組件名稱 | 功能與說明 | 人形<br>(人體器官對應) | 機器狗 | AMR |
| :--- | :--- | :--- | :---: | :---: | :---: |
| **運算模組** | 處理器 (CPU/GPU/SoC) | 負責運行作業系統、演算法與 AI 模型。 | ●<br>(大腦晶片) | ● | ● |
| | 邊緣運算平台 (IPC/SBC) | 處理高層決策任務，如導航、路徑規劃與任務排程。 | ●<br>(大腦中樞層) | ● | ● |
| | 控制器 (DSP/MCU) | 將系統指令轉化為精準的關節物理動作。 | ●<br>(小腦+神經) | ● | ● |
| **感知模組**<br>*(外部感測)* | RGB 相機 | 視覺感知基礎，通常人形配備在頭部或胸前。 | ●<br>(眼睛) | ● | ● |
| | 深度相機 (RGB-D) | 主流有雙目/多目視覺、結構光與 ToF。人形多以雙目/多目為主。 | ●<br>(雙眼/空間感) | ● | ● |
...
| **感知模組**<br>*(內部感測)* | 一維力感測器 | 測量單一維度力/扭矩，適用於關節（尤其是線性關節）。 | ◉<br>(本體感覺) | ◉ | ◉ |

...
最後，當機器人基礎模型逐漸成形後，下一階段的瓶頸將會從「模型能不能完成任務」，逐漸轉向「模型能不能部署至不同機器人」，這不只涉及 CPU、GPU、NPU 等各項算力配置，也牽涉記憶體（Memory）、感測資料傳輸，以及軟硬體之間的協同設計。目前已可預想，未來機器人的 AI 運算架構將走向**混合式 AI（Hybrid AI）**發展，在這脈絡下有三點值得思考的議題：
  ...
  1. **不應追求極致算力，而是在於「最佳分配」**
     ...
     不過，算力配置並非只是單純將不同任務交由適合的處理器執行這般簡單，還需從整體系統來看 CPU、GPU、NPU 如何協同運作，例如不同運算單元之間的記憶體存取與資料搬移，皆會影響系統延遲、頻寬占用與功耗表現，這也凸顯出一個事實，即使單一晶片具備更高算力，若不同運算單元之間無法有效協作，也未必能轉化為整機效能。因此，未來機器人運算架構不應一味追求極致算力，而是如何依據不同任務需求，讓 CPU、GPU、NPU 各司其職並有效協作，至於究竟「什麼才是最佳化分配？」，依然是大哉問，現今產業仍無定論及最佳解。

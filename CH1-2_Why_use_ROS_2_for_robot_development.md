...
* **第六步：視覺化工具檢視**
    完成各項設定後，可以使用 ROS 2 的三維視覺化工具 (主流如 RViz2) 進行視覺化檢查，但要留意 RViz2 適合顯示點雲、影像或坐標系等空間資料，若需觀察非空間的數值型資料（例如 IMU、GPS、電池電壓等會隨時間變化的數值），則需搭配像是 `rqt_plot` 的插件進行檢視。<br>而透過視覺化工具，有助於開發者檢查各個感測器資料，除了確認資料能正常顯示、也檢視各感測器之間的座標轉換都正確，以及感測器安裝位置與量測結果是符合預期的；待所有細節皆確認無誤後，再將感測資料整合至其他演算法，測試機器人在實際環境中的任務功能，就算完成機器人感知系統運作正常且符合需求的最後驗證工作。

...
首要之務是建立工作空間（Workspace），用來集中管理自行開發的 ROS 2 套件。例如先建立一個名為`ros2_ws`的工作空間，並在其中建`src`目錄，然後本例假設要建立的套件名稱為`ipcam`，接著到`src`目錄底下輸入終端機指令`ros2 pkg create --build-type ament_python ipcam`，即會產生該套件目錄結構；另外，對於 Python 型態的 ROS 2 套件而言，`ipcam` 套件內還會有另一個同名的 Python 模組目錄，因此節點程式`rtsp_camera_node.py`會位於`~/ros2_ws/src/ipcam/ipcam/rtsp_camera_node.py`，其工作空間與套件目錄結構如下：
...
不過，也不是所有節點發布的資料都適合透過 RViz2 進行驗證，RViz2 主要用於視覺化影像、點雲、座標轉換等資料，若節點發布的是純文字、數值或其他不適合視覺化的訊息，便可改用 ROS 2 提供的命令列工具進行確認，像是輸入`ros2 topic echo`指定 Topic 名稱，並直接在終端機中持續顯示該 Topic 所發布的 Message 內容。

---

完成上述步驟後，可以發現 ROS 2 套件的主要目的，是將節點程式、相依資訊與相關資源依照標準結構進行組織，使 ROS 2 能進一步辨識、建置、安裝與執行這些軟體功能；因此，當一個零組件沒有原廠或第三方提供的 ROS 2 套件時，不代表它無法進入 ROS 2，只是需要開發者自行透過 Python、C++ 或其他方式編寫節點程式，再依照 ROS 2 規範將程式封裝成套件，就能讓原本各自獨立的硬體進入 ROS 2 的開發框架。
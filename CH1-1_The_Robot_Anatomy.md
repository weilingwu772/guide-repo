...
| | 全景相機 | 整合多顆魚眼或廣角鏡頭，透過影像拼接技術，現已可提供 360° 大範圍視野，常用於彌補主視覺視野的不足（如機器人移動時的視覺盲區）。 | ◉<br>(眼角餘光) | ◉ | ◉ |
...
| | GNSS / RTK | 接收 GNSS 衛星訊號，搭配 RTK（Real-Time Kinematic，即時動態定位）技術，為戶外機器人提供公分級的絕對定位資訊；然而訊號易受遮蔽物及電磁干擾（EMI）影響，因此其天線需配置於機體頂部並盡可能遠離馬達或電源模組，機構設計上需特別留意。 | ○ | ◉ | ●<br>(戶外用) |
...
- **國際主流廠**：
  - **RGB相機**：Leopard Imaging(美/US)、e-con Systems(美/US)、海康威視(中/CN)
...
- **未來關鍵**：為了在算力需求與物理極限之間取得平衡，運算架構正逐漸從「控制器 - 邊緣運算平台」演進為「控制器 - 邊緣運算平台 - 高效能運算 (HPC)」三層協作體系。這源於智慧機器人（尤其是人型機器人與機器狗）對本體空間、重量配比、功耗與散熱等面向的極度要求，難以單靠機載邊緣運算平台運行參數龐大的AI模型與訓練技術——例如強化學習 (Reinforcement Learning，簡稱 RL)、視覺-語言-動作 (Vision-Language-Action，簡稱 VLA) 或世界模型 (World Model)；因此，獨立將大模型訓練與模擬 (Sim-to-Real) 交給HPC層處理，才能讓本體專注於即時控制，又同時滿足機器人訓練上的AI運算需求。

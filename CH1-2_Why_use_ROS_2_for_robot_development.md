...
3.  **第三步：物理安裝**
    下一步則是將它安裝在適當位置，並確保供電與通訊的線路穩固，同時要避免感測範圍受到其他機載裝置遮擋，以減少感知盲區；此外，若已知該智慧機器人未來的工作環境會遭遇到的環境課題，例如雨淋、風沙、不平整路面，則應搭配對應的防護設計(防水、防塵、減震等)，以提升感測資料品質。

4.  **第四步：標定校準**
    透過標定程序修正感測器本身的測量誤差，並建立各感測器之間的相對位置與姿態關係。
    *   內參標定 (Intrinsic Calibration)：校正鏡頭焦距、主點位置及鏡頭鏡片畸變，使影像能正確反映實際場景的幾何關係。
    *   外參標定 (Extrinsic Calibration)：計算各感測器相對於基準座標系（通常以主感測器如 3D LiDAR，或機器人本體中心 base_link）的相對位置與偏角，建立空間對位關係。

5.  **第五步：資料過濾**
    在感測資料送入演算法之前，還需先進行資料過濾，例如去除離群點、下採樣（Voxel Grid）、限制感測範圍（ROI Filtering）及移除雜訊濾波等，以降低運算負擔並提升後續演算法的穩定性與效率。

...
隨著智慧機器人的應用日益多元，開發工作已不再只是撰寫控制程式，而是涵蓋感測器整合、演算法開發、系統通訊、硬體平台選擇，以及不同廠商模組的協同運作。在這樣的需求下，ROS 2 憑藉其標準化通訊機制、模組化架構與完善的開源生態，逐漸成為現代機器人開發的主流框架。本章將從 ROS 2 的架構出發，進一步說明其在機器人感知系統、整機設計、零組件生態，以及 NVIDIA 與 Intel 等硬體平台上的應用，說明 ROS 2 為何已成為智慧機器人開發不可或缺的核心技術。

```mermaid
graph TD
    Community["🌐 開源社群 (Community)<br>GitHub / ROS Index"]
    Component["🧑 零組件商 (Component)<br>Sensor / Driver"]
    OEM["🧑 整機廠商 (OEM)<br>Integration / Deployment / OTA"]
    Eco["📂 ROS 2 生態系 (ROS 2 Ecosystem)<br>DDS / tf2 / Nav2 / RViz"]
    Compute["📂 運算平台 (Compute)<br>Jetson / Intel"]
    Products["🤖 機器人產品 (Robot Products)<br>AMR / Delivery / Service"]

    %% 縱向價值流 (Value Chain)
    Community -->|開放原始碼| Eco
    Eco -->|系統運行載體| Compute
    Compute -->|硬體軟體整合| Products

    %% 橫向角色交匯 (Value Network)
    Component -->|提供硬體與驅動| Eco
    Eco -->|軟體整合與部署| OEM
